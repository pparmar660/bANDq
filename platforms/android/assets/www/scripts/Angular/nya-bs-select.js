nyaBsSelect.directive('nyaBsSelect', ['$parse', '$document', '$timeout', '$compile', 'nyaBsConfig', function ($parse, $document, $timeout, $compile, nyaBsConfig) {
        ...
        var DEFAULT_NONE_SELECTION = 'Please Select';
        ...
        // ranjan edit for custom message in selection field


        var exText = null,exActive = false;
        
        var offenderExText=null,offenderActive=false;
                

        // ranjan edit ends here
        return {
            restrict: 'ECA',
            ...
            compile: function nyaBsSelectCompile(tElement, tAttrs) {
                ...
                if (tAttrs.ngChange == "updateLinkedStaff()") {
                    //console.log(tAttrs.class);
                    exActive = true;
                    exText = tAttrs.class.split(" ")[0];
                    //console.log(exText);

                }
                
                //added for getting model value and multiple select togather
                 else{
                     //console.log(tAttrs.class);
                    exActive = true;
                    exText = "";
...
//                                    debugger;
                                        if (localizedText.numberItemSelectedTpl) {
                                            ...
                                            if($element.attr("totalcount")!=null){
                                                localizedText.numberItemSelected = "%d of "+ $element.attr("totalcount") + " Selected";
                                            }else{
                                            localizedText.numberItemSelected = "%d " + exText + " Selected";
                                        }
                                    ...
                                    }   // ranjan edit for custom message in selection field ends here
                                    else {
                                        if (localizedText.numberItemSelectedTpl) {
                                            filterOption.append(jqLite(localizedText.numberItemSelectedTpl.replace('%d', viewValue.length)));
                                        } else if (localizedText.numberItemSelected) {
